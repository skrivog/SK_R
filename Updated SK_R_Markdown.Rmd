...
teo_snp_counts <- as.data.frame(table(teo_df_sorted$Chromosome))
colnames(teo_snp_counts) <- c("Chromosome", "SNP_Count")
teo_snp_counts$Chromosome <- factor(teo_snp_counts$Chromosome, levels = c(as.character(1:10), "multiple", "unknown"), ordered = TRUE)
...
maize_df_sorted$Chromosome <- factor(maize_df_sorted$Chromosome, levels = chromosome_levels)

ggplot(maize_df_sorted, aes(x = as.numeric(Position))) + geom_density(fill = "blue", alpha = 0.5) + facet_wrap(~ Chromosome, scales = "free_x", ncol = 5) + labs(title = "Density of SNP Positions in Maize by Chromosome", x = "Genomic Position", y = "Density") + theme_minimal() + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(teo_df_sorted, aes(x = as.numeric(Position))) + geom_density(fill = "blue", alpha = 0.5) + facet_wrap(~ Chromosome, scales = "free_x", ncol = 5) + labs (title = "Density of SNP Positions in Teosinte by Chromosome", 
...
process_and_plot(maize_df_sorted, "Maize")
process_and_plot(teo_df_sorted, "Teosinte")
