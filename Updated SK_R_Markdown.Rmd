...
teo_df_sorted$Chromosome <- factor(teo_df_sorted$Chromosome, levels = c(as.character(1:10), "multiple", "unknown"), ordered = TRUE)

ggplot(teo_df_sorted, aes(x = Chromosome, y = SNP_Count, fill = Chromosome)) +
  geom_bar(stat = "identity") +
...
maize_df_sorted$Chromosome <- factor(merged_data_subset$Chromosome, levels = chromosome_levels)
